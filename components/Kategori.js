 ...
 * Kategori.js
 * -----------
 * CRUD Kategori - tabel data + modal box tambah/edit.
 */
...
const Kategori = {
  template: `
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div class="flex flex-wrap justify-between items-center gap-3 mb-6">
        <div>
          <h1 class="text-2xl font-display">Kategori Buku</h1>
          <p class="text-ink/60 text-sm mt-1 font-medium">Kelola genre/kategori koleksi buku & komik.</p>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full">
            <thead class="bg-ink text-base">
              <tr>
                <th class="px-5 py-3 text-left text-xs font-bold uppercase">Nama Kategori</th>
                <th class="px-5 py-3 text-left text-xs font-bold uppercase">Deskripsi</th>
                <th class="px-5 py-3 text-right text-xs font-bold uppercase">Aksi</th>
              </tr>
            </thead>
            <tbody class="divide-y-[3px] divide-ink">
              <tr v-if="loading"><td colspan="3" class="px-5 py-8 text-center font-bold text-ink/50">Memuat data...</td></tr>
              <tr v-else-if="items.length === 0"><td colspan="3" class="px-5 py-8 text-center font-bold text-ink/50">Belum ada kategori.</td></tr>
              <tr v-for="item in items" :key="item.id" class="hover:bg-yolk/30">
                <td class="px-5 py-3.5 font-bold">{{ item.nama_kategori }}</td>
                <td class="px-5 py-3.5 text-ink/70 font-medium">{{ item.deskripsi || '-' }}</td>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-ink/60 flex items-center justify-center px-4 z-50">
        <div class="brut bg-panel w-full max-w-md p-6">
          <h2 class="text-lg font-display mb-4">{{ form.id ? 'Edit' : 'Tambah' }} Kategori</h2>
  ...
  methods: {
    ...
    openModal(item = null) {
      this.errorMsg = '';
      this.form = item
        ? { id: item.id, nama_kategori: item.nama_kategori, deskripsi: item.deskripsi }
        : { id: null, nama_kategori: '', deskripsi: '' };
      this.showModal = true;
    },
    async handleSubmit() {
      ...
      try {
        if (this.form.id) {
          await api.put(`/kategori/${this.form.id}`, this.form);
        } else {
    ...
    async handleDelete(item) {
      const confirm = await Swal.fire({
        ...
        text: `"${item.nama_kategori}" akan dihapus permanen.`,
        icon: 'warning',
