 ...
 * Anggota.js
 * ----------
 * CRUD Anggota - data member peminjam buku.
 */
...
const Anggota = {
  template: `
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div class="flex flex-wrap justify-between items-center gap-3 mb-6">
        <div>
          <h1 class="text-2xl font-display">Data Anggota</h1>
          <p class="text-ink/60 text-sm mt-1 font-medium">Kelola data anggota/member peminjam buku.</p>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full">
            <thead class="bg-ink text-base">
              <tr>
                <th class="px-5 py-3 text-left text-xs font-bold uppercase">Nama Anggota</th>
                <th class="px-5 py-3 text-left text-xs font-bold uppercase">No HP</th>
                <th class="px-5 py-3 text-left text-xs font-bold uppercase">Alamat</th>
            ...
            <tbody class="divide-y-[3px] divide-ink">
              ...
              <tr v-else-if="items.length === 0"><td colspan="4" class="px-5 py-8 text-center font-bold text-ink/50">Belum ada data anggota.</td></tr>
              <tr v-for="item in items" :key="item.id" class="hover:bg-yolk/30">
                <td class="px-5 py-3.5 font-bold">{{ item.nama_anggota }}</td>
                <td class="px-5 py-3.5 text-ink/70 font-medium">{{ item.no_hp || '-' }}</td>
                <td class="px-5 py-3.5 text-ink/70 font-medium">{{ item.alamat || '-' }}</td>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-ink/60 flex items-center justify-center px-4 z-50">
        <div class="brut bg-panel w-full max-w-md p-6">
          <h2 class="text-lg font-display mb-4">{{ form.id ? 'Edit' : 'Tambah' }} Anggota</h2>
          <form @submit.prevent="handleSubmit" class="space-y-4">
            <div>
              <label class="block text-sm font-bold mb-1">Nama Anggota</label>
              <input v-model="form.nama_anggota" required
  ...
  data() {
    return {
      ...
      submitting: false,
      errorMsg: '',
      form: { id: null, nama_anggota: '', no_hp: '', alamat: '' },
    };
  },
  created() {
    this.fetchData();
  ...
  methods: {
    ...
    openModal(item = null) {
      this.errorMsg = '';
      this.form = item
        ? { id: item.id, nama_anggota: item.nama_anggota, no_hp: item.no_hp, alamat: item.alamat }
        : { id: null, nama_anggota: '', no_hp: '', alamat: '' };
      this.showModal = true;
    },
    async handleSubmit() {
      ...
      try {
        if (this.form.id) {
          await api.put(`/anggota/${this.form.id}`, this.form);
        } else {
    ...
    async handleDelete(item) {
      const confirm = await Swal.fire({
        title: 'Hapus Anggota?',
        text: `"${item.nama_anggota}" akan dihapus permanen.`,
        icon: 'warning',
